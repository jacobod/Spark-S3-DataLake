...
def process_song_data(spark, input_data, output_data):
    ...
    song_data = input_data    #"s3://udacity-dend/song_data/*.json"

    # read song data file
    #sc = SparkContext.getOrCreate(SparkConf().setMaster("local[*]"))
    #text_rdd = sc.textFile(os.path.join(input_data, "*"))
    df = spark.read.json(song_data)
    df.printSchema()

    # create global view for querying
    #df.createGlobalTempView("songs_ext")
...
def process_log_data(spark, input_data, output_data):
    ...
    song_df = spark.read.json("./Data/song_data/")
...
def main():
    ...
    process_song_data(spark, input_data+"song_data/A/A/A/TRAAAAW128F429D538.json", output_data) #Documents/UdacityDataEngineering/SparkProject/Data/song_data/A/A/A/TRAAAAW128F429D538.json
    process_log_data(spark, input_data+"log_data", output_data)
